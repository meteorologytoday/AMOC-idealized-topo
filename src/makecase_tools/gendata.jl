...
EmPmR .*= parsed["empmr-north-boundary"] / EmPmR[2, end-1]
...
open(joinpath(parsed["output-dir"], "caseinfo.txt"), "w") do io
    write(io, "Freshwater flux at north : $(format("{:.3e}", parsed["empmr-north-boundary"])) m/s\n")
    write(io, "Total freshwater flux  : $(format("{:.5f}", total_water_flux)) Sv\n")
end

...
Dataset(parsed["data-file"]) do ds
    ...
    global WVEL  = nomissing(ds["WVEL"][:, :, :, :], NaN) 
    global UVEL  = nomissing(ds["UVEL"][:, :, :, :], NaN) 
...
Dataset(parsed["output-file"], "c") do ds
    ...
    for (varname, vardata, vardim, attrib) in [
        ...
        ("BUOY", BUOY, ("X", "Y", "Z", "time",), Dict()),
        ("UVEL", UVEL, ("Xp1", "Y", "Z", "time",), Dict()),
        ("VVEL", VVEL, ("X", "Yp1", "Z", "time",), Dict()),
        ("WVEL", WVEL, ("X", "Y", "Z", "time",), Dict()),
        ("THETA", THETA, ("X", "Y", "Z", "time",), Dict()),
        ("SALT", SALT, ("X", "Y", "Z", "time",), Dict()),
    ]
